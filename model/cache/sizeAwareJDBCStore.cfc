...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->
	...
	<!--- init --->
	<cffunction name="init" access="public" output="false" returntype="sizeAwareJDBCStore" hint="Constructor">
		...
		<cfscript>
			// Store Fields
			var fields = "objectKey,hits,timeout,lastAccessTimeout,created,lastAccessed,isExpired,isSimple,size";
			...
			// Indexer
			instance.indexer = createObject("component","model.cache.sizeAwareJDBCMetadataIndexer").init(fields, config, this);
	...
	<cffunction name="set" access="public" output="false" returntype="void" hint="sets an object in the storage.">
		...
		<cftransaction>
			<cfif NOT lookupQuery( arguments.objectKey ).recordcount>
				<!--- store it --->
				<cfquery name="q" datasource="#instance.dsn#" username="#instance.dsnUsername#" password="#instance.dsnPassword#">
				INSERT INTO #instance.table# (id,objectKey,objectValue,hits,timeout,lastAccessTimeout,created,lastAccessed,isExpired,isSimple,size)
				     VALUES (
						...
						<cfqueryparam cfsqltype="cf_sql_integer" value="#len(arguments.object)#">
					 )
				</cfquery>
				<!--- Just go back --->
				<cfreturn>
			...
			UPDATE #instance.table#
			   SET  objectKey 			= <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.objectKey#">,
					...
					size				= <cfqueryparam cfsqltype="cf_sql_integer" value="#len(arguments.object)#">
			  WHERE id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#normalizedID#">
			</cfquery>
		...
		<cfif NOT tableFound>
			...
			<cfquery name="qCreate" datasource="#instance.dsn#" username="#instance.dsnUsername#" password="#instance.dsnPassword#">
				CREATE TABLE #instance.table# (
					...
					size #create.intType# NOT NULL DEFAULT '0',
					PRIMARY KEY (id)
				) #create.afterCreate#
			</cfquery>
		</cfif>
